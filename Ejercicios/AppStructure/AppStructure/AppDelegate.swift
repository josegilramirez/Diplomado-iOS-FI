...
//  AppStructure
//
//  Created by José Gil Ramírez S on 12/10/18.
...
class AppDelegate: UIResponder, UIApplicationDelegate {
    ...
    func application(_ application: UIApplication, didFinishLaunchingWithOptions launchOptions: [UIApplicationLaunchOptionsKey: Any]?) -> Bool {
        ...
        let testView1 = UIView(frame: CGRect(x: 0, y: 0, width: 200, height: 300))
        testView1.backgroundColor = UIColor.gray
        rootViewController.view.addSubview(testView1)
        //window = UIWindow(frame: UIScreen.main.bounds)
        window = UIWindow(frame: CGRect(x: 0, y: 0, width: 300, height: 600))
        window?.rootViewController = rootViewController
        let testView2 = UIView(frame: CGRect(x: 0, y: 0, width: 100, height: 200))
        testView2.backgroundColor = UIColor.blue
        window?.addSubview(testView2)
